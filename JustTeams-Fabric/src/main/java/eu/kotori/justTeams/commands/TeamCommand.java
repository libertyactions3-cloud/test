...
public final class TeamCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("team")
                ...
                .then(CommandManager.literal("create").then(CommandManager.argument("name", StringArgumentType.word()).then(CommandManager.argument("tag", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_CREATE, () -> create(c.getSource(), StringArgumentType.getString(c, "name"), StringArgumentType.getString(c, "tag")))))))
                ...
                .then(CommandManager.literal("warp")
                        ...
                        .then(CommandManager.argument("name", StringArgumentType.word())
                                ...
                                .then(CommandManager.argument("password", StringArgumentType.word()).executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_WARP, () -> useWarp(c.getSource(), StringArgumentType.getString(c, "name"), StringArgumentType.getString(c, "password")))))))
                .then(CommandManager.literal("invite").then(CommandManager.argument("player", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_INVITE, () -> invite(c.getSource(), StringArgumentType.getString(c, "player"))))))
                .then(CommandManager.literal("accept").then(CommandManager.argument("team", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_ACCEPT, () -> acceptInvite(c.getSource(), StringArgumentType.getString(c, "team"))))))
                .then(CommandManager.literal("deny").then(CommandManager.argument("team", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_DENY, () -> denyInvite(c.getSource(), StringArgumentType.getString(c, "team"))))))
                .then(CommandManager.literal("join").then(CommandManager.argument("team", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_JOIN, () -> requestJoin(c.getSource(), StringArgumentType.getString(c, "team"))))))
                .then(CommandManager.literal("unjoin").then(CommandManager.argument("team", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_UNJOIN, () -> cancelJoinRequest(c.getSource(), StringArgumentType.getString(c, "team"))))))
    ...
    private static int run(ServerCommandSource source, String permission, CommandAction action) {
        try {
            if (source.getEntity() instanceof ServerPlayerEntity player && !JustTeamsFabric.permissions().has(player, permission)) {
    ...
    private static int leave(ServerCommandSource s) throws Exception {
        ...
        JustTeamsFabric.glow().stopGlowForPlayer(s.getServer(), p.getUuid());
        JustTeamsFabric.teams().removeMember(t, p.getUuid());
        JustTeamsFabric.storage().save(JustTeamsFabric.teams());
        TeamNotificationManager.notifyLeave(s.getServer(), t, p.getUuid());
        return 1;
    }
    ...
    private static int disband(ServerCommandSource s) throws Exception {
        ...
        for (TeamPlayer member : t.getMembers()) {
            ...
            JustTeamsFabric.glow().stopGlowForPlayer(s.getServer(), member.getPlayerUuid());
        }
        JustTeamsFabric.teams().unregister(t);
        JustTeamsFabric.storage().save(JustTeamsFabric.teams());
        TeamNotificationManager.notifyDisband(s.getServer(), t, p.getUuid());
        return 1;
    }
    ...
    private static int setHome(ServerCommandSource s, String name) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); TeamPlayer member = team.getMember(player.getUuid()); if (member == null || !member.canSetHome()) { s.sendError(Text.literal("You do not have permission to set the team home.")); return 0; } team.setHome(TeamLocation.fromPlayer(player)); JustTeamsFabric.storage().save(JustTeamsFabric.teams()); s.sendFeedback(() -> Text.literal("Team home set at your current location."), false); return 1; }
    private static int clearHome(ServerCommandSource s, String name) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); TeamPlayer member = team.getMember(player.getUuid()); if (member == null || !member.canSetHome()) { s.sendError(Text.literal("You do not have permission to clear the team home.")); return 0; } if (team.getHome() == null) { s.sendError(Text.literal("Your team does not have a home set.")); return 0; } team.clearHome(); JustTeamsFabric.storage().save(JustTeamsFabric.teams()); s.sendFeedback(() -> Text.literal("Team home cleared."), false); return 1; }
    private static int useHome(ServerCommandSource s) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); TeamPlayer member = team.getMember(player.getUuid()); if (member == null || !member.canUseHome()) { s.sendError(Text.literal("You do not have permission to use the team home.")); return 0; } if (team.getHome() == null) { s.sendError(Text.literal("Your team does not have a home set.")); return 0; } teleport(s, player, team.getHome()); return 1; }
    private static int setWarp(ServerCommandSource s, String name) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); TeamPlayer member = team.getMember(player.getUuid()); if (member == null || !member.canSetHome()) { s.sendError(Text.literal("You do not have permission to create team warps.")); return 0; } if (name.length() > 32) { s.sendError(Text.literal("Warp name must be 32 characters or fewer.")); return 0; } if (team.getWarp(name) != null) { s.sendError(Text.literal("A warp with that name already exists.")); return 0; } TeamLocation location = TeamLocation.fromPlayer(player); team.addWarp(new TeamWarp(name, player.getUuid(), location.getDimension(), location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch())); JustTeamsFabric.storage().save(JustTeamsFabric.teams()); s.sendFeedback(() -> Text.literal("Team warp '" + name + "' created."), false); return 1; }
    private static int removeWarp(ServerCommandSource s, String name) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); TeamWarp warp = team.getWarp(name); if (warp == null) { s.sendError(Text.literal("Warp not found.")); return 0; } if (!team.isOwner(player.getUuid()) && !warp.getOwner().equals(player.getUuid())) { s.sendError(Text.literal("Only the team owner or warp creator can remove this warp.")); return 0; } team.removeWarp(name); JustTeamsFabric.storage().save(JustTeamsFabric.teams()); s.sendFeedback(() -> Text.literal("Team warp '" + name + "' removed."), false); return 1; }
    private static int listWarps(ServerCommandSource s) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); if (team.getWarps().isEmpty()) { s.sendFeedback(() -> Text.literal("Your team has no warps."), false); return 1; } s.sendFeedback(() -> Text.literal("Team warps:"), false); for (TeamWarp warp : team.getWarps()) s.sendFeedback(() -> Text.literal("- " + warp.getName() + (warp.isEnabled() ? "" : " (disabled)")), false); return 1; }
    private static int useWarp(ServerCommandSource s, String name, String password) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team team = requireTeam(s, player); TeamWarp warp = team.getWarp(name); if (warp == null) { s.sendError(Text.literal("Warp not found.")); return 0; } if (!warp.isEnabled()) { s.sendError(Text.literal("That warp is disabled.")); return 0; } if (!warp.isMembersCanUse() && !team.isOwner(player.getUuid())) { s.sendError(Text.literal("You do not have permission to use that warp.")); return 0; } if (!warp.getPassword().isEmpty() && !warp.getPassword().equals(password)) { s.sendError(Text.literal("Incorrect warp password.")); return 0; } teleport(s, player, new TeamLocation(warp.getWorld(), warp.getX(), warp.getY(), warp.getZ(), warp.getYaw(), warp.getPitch())); return 1; }

    private static void teleport(ServerCommandSource source, ServerPlayerEntity player, TeamLocation location) throws Exception {
        Identifier identifier = Identifier.tryParse(location.getDimension()); if (identifier == null) throw new IllegalStateException("The saved team location has an invalid dimension: " + location.getDimension());
        RegistryKey<World> key = RegistryKey.of(RegistryKeys.WORLD, identifier); MinecraftServer server = source.getServer(); ServerWorld world = server.getWorld(key);
        if (world == null) throw new IllegalStateException("The saved team location's dimension is not available on this server.");
        player.teleport(world, location.getX(), location.getY(), location.getZ(), Set.of(), location.getYaw(), location.getPitch(), true); source.sendFeedback(() -> Text.literal("Teleported to the team location."), false);
    }
    ...
    private static int openRequests(ServerCommandSource s) throws Exception { ServerPlayerEntity player = s.getPlayerOrThrow(); Team t = JustTeamsFabric.teams().getTeam(player.getUuid()); if (t == null) { s.sendError(Text.literal("You are not in a team.")); return 0; } JoinRequestGui.open(player, t); return 1; }
