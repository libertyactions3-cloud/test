...
public final class TeamGuiManager {
    ...
    private static void handleMainClick(PlayerEntity player, int slot, int button, SlotActionType actionType, Team team, TeamMenuHandler menu) {
        ...
        switch (slot) {
            ...
            case 52 -> {
                if (team.hasElevatedPermissions(player.getUuid())) TeamSettingsGui.open(player, team);
                else player.sendMessage(Text.literal("Only the owner or co-owners can access team settings."), true);
            }
            case 8 -> {
    ...
    private static void close(PlayerEntity player) {
        if (player instanceof ServerPlayerEntity serverPlayer) serverPlayer.closeHandledScreen();
    }

    private static int memberIndexForSlot(int slot) {
        for (int i = 0; i < MEMBER_SLOTS.length; i++) if (MEMBER_SLOTS[i] == slot) return i;
        return -1;
    }
    ...
    private static void save() {
        try {
            ...
            JustTeamsFabric.LOGGER.error("Failed to save JustTeams data after GUI action", exception);
        }
    }

    @FunctionalInterface
    public interface TeamMenuActionHandler {
        void handle(PlayerEntity player, int slot, int button, SlotActionType actionType, Team team, TeamMenuHandler menu);
    }
}
