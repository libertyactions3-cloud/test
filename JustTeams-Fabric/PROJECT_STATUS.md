...
The user has explicitly requested that no clean build be performed until **Round 10**.
...
- Team glow
- Team Ender Chest implementation

## Known Gaps / Active Investigations

### Team Glow — ROUND 7 COMPLETE / RUNTIME VERIFICATION PENDING
...
- `/team glow` exists.
- It is gated by the `team_glow` feature configuration in the reference.
...
- There is no verified dedicated `justteams.command.glow` permission check in the reference command handler.
- The actual command calls `teamManager.toggleGlow(player)`.

The Fabric implementation now has the glow state, command integration, refresh mechanism, team-role coloring, and lifecycle cleanup paths that were established during the Round 7 work.

Do not regress the verified authorization model by inventing a dedicated glow command permission.

### Team Ender Chest — ROUND 8 ACTIVE

Reference behavior established from justTeams 2.5.3:

- `/team enderchest` and `/team ec` open a shared team-owned inventory.
- Ender Chest access is feature-configurable.
- A member needs `canUseEnderChest`, unless the `justteams.bypass.enderchest.use` permission bypasses the member restriction.
- The inventory is persistent and belongs to the team rather than an individual player.
- Inventory size is configurable by rows.
- Multiple team members can view the same inventory simultaneously.
- Viewers are tracked so the shared inventory can be released after the final viewer closes it.
- Inventory changes are persisted.
- The Paper reference also has distributed database locking and Redis cross-server synchronization.

Fabric implementation committed in Round 8 so far:

- `TeamEnderChest` shared inventory with viewer tracking and ItemStack CODEC/NBT serialization.
- `Team` now owns optional Ender Chest state.
- `TeamStorage` persists Ender Chest contents and bumped its data version to 5.
- `JustTeamsConfig` now exposes `enderchest.enabled` and `enderchest.rows` defaults.
- `TeamEnderChestGui` enforces team membership, `canUseEnderChest`, and the bypass permission.
- `TeamEnderChestScreenHandler` provides the configurable vanilla chest interface.
- Team GUI slot 46 now opens the real Team Ender Chest instead of the placeholder.
- `/team enderchest` command integration was added using `justteams.command.enderchest`.

Known deliberate Fabric-side difference for later documentation:

- The current port is single-server and has no Redis/database infrastructure, so the reference's cross-server distributed Ender Chest lock and Redis synchronization have not been ported. This must remain explicitly classified rather than silently treated as replicated.

Remaining Round 8 work:

1. Complete a code-level parity audit of Ender Chest access, aliases, persistence, viewer lifecycle, and GUI integration.
2. Verify the exact reference command alias behavior and permission semantics.
3. Check leave/disband/disconnect edge cases against the reference lifecycle.
4. Keep all implementation changes unbuilt until Round 10 as requested.
...
Round 7 implementation complete; runtime verification remains part of the final testing pass.

### B. Resolve Team Ender Chest placeholders

Reference behavior is established and implementation is underway in Round 8.
...
Include teams, membership, ownership/co-ownership, invitations, join requests, homes, warps, settings, PvP state, bank contents, glow state, Ender Chest contents, and other persisted state.
...
- friendly-fire state
- glow lifecycle
- Ender Chest concurrent viewers and close behavior
...
- **Round 7/10:** Completed the Fabric glow implementation and lifecycle integration. No clean build was run; runtime/build verification remains deferred to the final pass.
- **Round 8/10:** Began Team Ender Chest parity implementation: shared persistent inventory, configurable rows/enable flag, member/bypass permission enforcement, GUI integration, command integration, and NBT persistence were committed. Code-level parity audit remains active.

## Current Resume Point

**Continue Round 8: audit the newly implemented Team Ender Chest against the 2.5.3 call chain before moving to the next feature.**

Important constraint: **do not run the clean Gradle build until Round 10.**

Use resemblance/call-chain searching if the reference source cannot be found by exact name. If the reference source cannot be accessed or indexed sufficiently, tell the user exactly what reference is needed and why.
